...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        var_eps_sum_inv = 1 / np.sqrt(var + eps)
        x_mean_diff = (x - mean)
        x_n = var_eps_sum_inv * x_mean_diff
        out = gamma * x_n + beta

        cache = (x_n, x, gamma, var_eps_sum_inv, x_mean_diff)
...
def batchnorm_backward(dout, cache):
    ...
    x_n, x, gamma, var_eps_sum_inv, x_mean_diff = cache
    N = x.shape[0]
    dgamma = np.sum(dout * x_n, axis=0)
    dbeta = np.sum(dout, axis=0)

    dxhat = dout * gamma
    da = var_eps_sum_inv * dxhat
    db = x_mean_diff * dxhat
    dc = -(var_eps_sum_inv**2) * db
    de = 0.5 * var_eps_sum_inv * dc
    dsig = np.sum(de, axis=0)
    dmu = -var_eps_sum_inv * np.sum(dxhat, axis=0) - dsig * (2/N) * np.sum(x_mean_diff, axis=0)

    dx = da + (2*x_mean_diff/N) * dsig + (dmu/N)
