    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers + 1):
          ...
          if (i == self.num_layers):
            ...
            if self.use_batchnorm: 
              x, cache = affine_batchnorm_relu_forward(x, w, b, self.params['gamma{}'.format(i)], self.params['beta{}'.format(i)], self.bn_params[i-1])
            else:
        ...
        for i in range(self.num_layers, 0, -1):
          ...
          if (i == self.num_layers):
            ...
            if self.use_batchnorm:
              dout, grads[cur_w], grads[cur_b], grads['gamma{}'.format(i)], grads['beta{}'.format(i)] = affine_batchnorm_relu_backward(dout, caches.pop())
            else:
