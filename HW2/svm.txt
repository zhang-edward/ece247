...
class SVM(object):
  ...
  def loss(self, X, y):
    ...
    #   set margins, and then normalize the loss by the number of 
	  #	  training examples.)
    # ================================================================ #
      loss_array = 1 + X[i].dot(self.W.T) - X[i].dot(self.W.T)[y[i]]
      loss_array[loss_array < 0] = 0
      loss += np.sum(loss_array, axis=0) - 1  # subtract one to get rid of the j = y(i) case      
    loss /= num_train
    # ================================================================ #
    # END YOUR CODE HERE
    # ================================================================ #
  ...
  def fast_loss_and_grad(self, X, y):
    ...
    zj = (scores.T - scores[np.arange(num_train), y] + 1).T
    zj[zj < 0] = 0
    zj[np.arange(num_train),y] = 0
    loss = np.sum(zj) / num_train
    ...
    mask = np.zeros(zj.shape)
    mask[zj > 0] = 1
    mask[np.arange(num_train), y] = -np.sum(mask, axis=1)

    grad = mask.T.dot(X) / num_train
