...
# ================================================================ #
    ...
    ea = np.exp(scores - np.max(scores))
    sums = np.sum(ea, axis=1)
...
# ================================================================ #
    ...
    grad_softmax /= N
    grads['W2'] = grad_softmax.T.dot(h1) + reg * W2   # (C x N) * (N x H) = (C x H)
    grads['b2'] = np.sum(grad_softmax, axis=0)        # (C, )
    # (H x C) * (C x N) = (H x N)
    # (H x N) * (N x D) = (H x D)
    ind = h1
    ind[ind > 0] = 1
    ind[ind <= 0] = 0
    grads['W1'] = (W2.T.dot(grad_softmax.T) * ind.T).dot(X) + reg * W1
    grads['b1'] = np.sum((W2.T.dot(grad_softmax.T) * ind.T).T, axis = 0)
  ...
  def predict(self, X):
    ...
    scores = np.maximum(X.dot(self.params['W1'].T) + self.params['b1'], 0).dot(self.params['W2'].T) + self.params['b2']
