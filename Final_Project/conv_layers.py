    ...
    for f in range(F):
      for i in range(0, H_new):
        for j in range(0, W_new):
          i_start = i * stride
          j_start = j * stride
          x_patch = xpad[n, :, i_start:i_start+HH, j_start:j_start+WW]
          conv = np.sum(np.multiply(x_patch, w[f])) + b[f]
          out[n, f, i, j] = conv
    ...
    for f in range(F):
      ...
      for i in range(out_height):
        for j in range(out_width):
          i_start = i * stride
          j_start = j * stride
          i_end = i_start + f_height
          j_end = j_start + f_width
          x_patch = xpad[n, :, i_start:i_end, j_start:j_end]

          dx[n, :, i_start:i_end, j_start:j_end] += w[f] * dout[n, f, i, j]
